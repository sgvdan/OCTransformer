...
def load_backbone(model_name):
    if "imagenet" in str(model_name).lower():
        ...
        print("Loading model from Kermany for SLIVER-NET")
...
def nonadaptiveconcatpool2d(x, k):
    ...
    ap = torch.nn.functional.avg_pool2d(x, kernel_size=(k, k), stride=(k, k))
    mp = torch.nn.functional.max_pool2d(x, kernel_size=(k, k), stride=(k, k))
...
class SliverNet2(torch.nn.Module):
    ...
    def forward(self,x,cov=None):
        ...
        batch_size, slices_num, channels_num, height, width = x.shape
        x = x.permute(0, 2, 1, 3, 4).reshape(batch_size, channels_num, slices_num*height, width)
