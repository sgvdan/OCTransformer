...
for i, (images, labels) in enumerate(test_loader):
    ...
    for cam_algo in cams:
        ...
        superimposed_img = heatmap * 0.4 + images.squeeze().permute(1, 2, 0).cpu().detach().numpy().shape
        superimposed_img *= 255.0 / superimposed_img.max()
        res.append(superimposed_img / 255)
    gradcam = res
    row = [i, wandb.Image(images), predicted.item(), labels.item(),
